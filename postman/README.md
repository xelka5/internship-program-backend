...
**!!!Important: In order to successfully execute tests agains dev environment (currently only supported environment), please build and run dev containers on the main app (read in the root readme file how to run them). This compose contains connection to the dev network in order to execute the tests against the dev api service.**